    ...
    <div class="offer-letters-mgmt">
      ...
      <mat-card class="table-card">
        <mat-card-content>
          ...
          <div class="table-container" *ngIf="!loading && offers.length > 0">
...
export class OfferLettersManagementComponent implements OnInit {
  ...
  /** Selects the top N GENERATED offers by score, across the whole eligible set — not just
   *  the current page — via the lightweight generated-ids endpoint. */
  selectTop(n: number): void {
    this.offerService.getGeneratedIdsSortedByScore().subscribe({
      next: (ids) => {
        this.selected.clear();
        ids.slice(0, n).forEach((id) => this.selected.add(id));
      },
      error: () => this.toastService.error('Failed to load eligible offers for selection.'),
    });
  }

  /** Selects every GENERATED offer system-wide (not just the current page). */
